...
(defpackage :jkit.base.unify-global-var (:use))

(jkit.core:define-package :jkit.base.unify* (:jkit.base.unify)
  (:use :cl)
  (:import/export :jkit.base.hash-table*)
  (:export 
...
;@eval-always
(defvar *pkg-unify-global-var* (find-package :jkit.base.unify-global-var))
...
(defmacro internal-label (x) 
  `(intern (concatenate 'string "_unif_label_" (symbol-name ,x)) :jkit.base.unify))
...
(defun special-post-code (ptn dst var info &aux (h (car ptn)))
  ...
  (acond ((or (eq :member h)
        ...
        ((member h '(:length= :length/= :length< :length<= :length> :length>=))
         `(and 
           (typep ,var 'sequence)
           (,(intern (symbol-name h) :jkit.base.unify) ,(second ptn) ,var)))
           
        ((member h '(:length<< :length<=< :length<=<= :length<<=))
         `(and 
           (typep ,var 'sequence)
           (,(intern (symbol-name h) :jkit.base.unify) ,(second ptn) ,(third ptn) ,var)))
         ...
         (if is-a-cons?
           ...
           ((eq :list* head)
             (unless (and (proper-list-p (cdr ptn))
                          (<= 2 (length (cdr ptn))))
               (error "LIST*"))
             (let* ((x (intern "!jkit.base.unify-list*-tmpvar!"))
                    (x? (memoized (intern "??!jkit.base.unify-list*-tmpvar!"
