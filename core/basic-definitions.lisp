...
(defpackage :jkit.mspace
  (:use :cl) ;; <<- これをしないとCCLでエラーがでる。シンボル|@|について競合するとかなんとか…
  (:nicknames :jkit.core.monolithic-name-space))

(jkit.core.init:define-package :jkit.core.basic-definitions ()
  (:use :cl :jkit.core.init :anaphora :cl-annot)

  (:import/export :jkit.core.primary-annotations)
...
(in-package :jkit.core.basic-definitions)
...
(defparameter *<pkg/mspace>* (find-package :jkit.mspace))
