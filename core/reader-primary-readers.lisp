...
(defun <register-primary-readers-for-mspace> ()
  ...
  ;;    (let (result
  ;;          (cur-pkg *package*))
  ;;      (let* ((*package* (jkit.base:memoized (find-package :jkit.mspace)))
  ;;             (*readtable* CL-JKit.LE.CORE:*QI-READTABLE*)
  ;;             (src (read stream t nil t)))
  ;;        (setf result (if (CL-JKit.LE.CORE::%need-trans-p% src)
  ;;                       (let ((*package* cur-pkg))
  ;;                         (CL-JKit.LE.CORE::%trans% src))
  ;;                       src)))
  ;;      result)))
  ...
  (set-dispatch-macro-character
   ...
   (lambda (stream char1 char2)
     ...
     (let ((*package* (memoized (find-package :jkit.mspace)))
