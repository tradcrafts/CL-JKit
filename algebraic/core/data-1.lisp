...
(in-package :jkit.algebraic.core)
...
;; global: sym -> CL-JKit.MSPACE::|@sym| , QSPACE::|sym#|
;; local: sym-> @@sym , <sym#>

(defun <make-xi-ident> (sym &optional local?)
  (let* ((name (symbol-name sym))
         (xi-base-name (string-downcase sym))
         (q-pkg (memoized (find-package :jkit.mspace))))
    (if local?
...
(defmethod print-object ((x DATATUPLE) stream)
  (let* ((type (type-of x))
         (arity (get type 'jkit.algebraic.xdata::|%data_arity%|))
         (data (xtuple-snd x)))
    (case arity
  ...
  (cond ((symbolp d)
          (multiple-value-bind (q-ident q-tag) (<make-xi-ident> d local?)
            (add `(defclass ,d ,(if superclass superclass '(DATATUPLE)) ())
                 `(setf (get ',q-tag 'jkit.algebraic.xdata::|%classdata%|) ',d)
                 `(setf (get ',q-ident 'jkit.algebraic.xdata::|%data_arity%|) 0)
                 `(setf (get ',q-ident 'jkit.algebraic.xdata::|%data_tag%|) ',q-tag)
                 `(setf (get ',d 'jkit.algebraic.xdata::|%data_arity%|) 0) ;;
                 `(setf (get ',d 'jkit.algebraic.xdata::|%data_tag%|) ',q-tag) ;;
        ...
        ((consp d)
          ...
          (add `(defmethod deconstructable-p ((a ,c)) (declare (ignore a)) t)
               ...
               `(setf (get ',q-tag 'jkit.algebraic.xdata::|%classdata%|) ',c)
               `(setf (get ',q-ident 'jkit.algebraic.xdata::|%data_arity%|) ,arity)
               `(setf (get ',q-ident 'jkit.algebraic.xdata::|%data_tag%|) ',q-tag)
               `(setf (get ',c 'jkit.algebraic.xdata::|%data_arity%|) ,arity)
               `(setf (get ',c 'jkit.algebraic.xdata::|%data_tag%|) ',q-tag)
